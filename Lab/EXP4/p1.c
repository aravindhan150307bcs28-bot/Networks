...
[24bcs163@mepcolinux EXP3]$^C
[24bcs163@mepcolinux EXP3]$^C
[24bcs163@mepcolinux EXP3]$cd ..
[24bcs163@mepcolinux NETWORKS]$cd EXP4
[24bcs163@mepcolinux EXP4]$ls
bell.c  EXP4.prn  p1.c
[24bcs163@mepcolinux EXP4]$cat bell.c 
#include <stdio.h>
#define MAX 20
#define INF 9999

struct Node{char name;int distance;int parent;};
...
void initialize(int v,int s){
    for(int i=0;i<v;i++){node[i].distance=INF;node[i].parent=-1;}
    node[s].distance=0;
}
void relax(int u,int v,int w){
    if(node[u].distance!=INF && node[u].distance+w<node[v].distance){
...
int bellmanFord(int vertices,int edges,int source){
    ...
    for(int i=1;i<=vertices-1;i++)
      for(int j=0;j<edges;j++)
        relax(edge[j].src,edge[j].dest,edge[j].weight);
    for(int j=0;j<edges;j++){
      int u=edge[j].src,v=edge[j].dest,w=edge[j].weight;
      if(node[u].distance!=INF && node[u].distance+w<node[v].distance) return 0;
    }
    return 1;
}
void printPath(int vertex){
    struct PathNode pool[MAX];
    int used=0;
    struct PathNode *head=NULL,*temp;
    while(vertex!=-1){
        temp=&pool[used++];
        temp->vertex=vertex;
        temp->next=head;
        head=temp;
        vertex=node[vertex].parent;
    }
    temp=head;
    while(temp){
        printf("%c",node[temp->vertex].name);
        if(temp->next) printf(" -> ");
        temp=temp->next;
    }
}
void printResult(int vertices){
    ...
    for(int i=0;i<vertices;i++){
        ...
        if(node[i].distance==INF) printf("INF\t\t-\n");
        else{ printf("%d\t\t",node[i].distance); printPath(i); printf("\n");}
    }
}
int main(){
 ...
 if(vertices>MAX||edges>MAX){printf("Maximum allowed vertices and edges is %d\n",MAX);return 0;}
 printf("\nEnter Vertex Names:\n");
 for(int i=0;i<vertices;i++){printf("Vertex %d: ",i);scanf(" %c",&node[i].name);}
 printf("\nEnter Edges (Source Destination Weight)\nUse Vertex Numbers (0 to %d)\n\n",vertices-1);
 for(int i=0;i<edges;i++){
   printf("Edge %d: ",i+1);
   scanf("%d%d%d",&edge[i].src,&edge[i].dest,&edge[i].weight);
   if(edge[i].src<0||edge[i].src>=vertices||edge[i].dest<0||edge[i].dest>=vertices){printf("Invalid Edge!\n");return 0;}
 }
 printf("\nEnter Source Vertex Number: "); scanf("%d",&source);
 if(source<0||source>=vertices){printf("Invalid Source Vertex\n");return 0;}
 if(bellmanFord(vertices,edges,source)) printResult(vertices);
